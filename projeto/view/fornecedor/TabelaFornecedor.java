//Esse cara aqui que vai usar pra chama o frame e mostrar a tela 
//MOSTRA PRA NOS, TODOS OS DADOS CADASTRADOS LA NO BD
...
public class TabelaFornecedor extends JInternalFrame {
	private static final long serialVersionUID = -8245569444188473256L;
	...
	private JTextField textFieldBuscarNomeFornecedor;
	...
	private void iniciarPaginacao() {
			List<Fornecedor> listaFornecedor = new ArrayList<Fornecedor>();//armazenar as informacoes que vai ser lida pelo usuario
			totalRegistros = buscarTotalRegistrosFornecedores();
			...
			//LOGICA DE BLOQUEAR OU LIBERAR OS BOTOES
				...
				listaFornecedor = carregarListaFornecedor(paginaAtual, registrosPorPagina);	
				
				tabelaFornecedorModel.setListFornecedor(listaFornecedor);
				tabelaFornecedorDesign.setModel(tabelaFornecedorModel);	//adiciona o modelo
				tabelaFornecedorDesign.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
				
				sortTabelaFornecedor = new TableRowSorter<TabelaFornecedorModel>(tabelaFornecedorModel);
				tabelaFornecedorDesign.setRowSorter(sortTabelaFornecedor);
				tabelaFornecedorModel.fireTableDataChanged();  //pra atualizar a tabela caregando os dados
				tabelaFornecedorDesign.setAutoCreateRowSorter(true); //cada coluna eh autoclassificada, dai clica na coluna e dale
				
				tabelaFornecedorDesign.getColumnModel().getColumn(CODIGO).setPreferredWidth(11);
				tabelaFornecedorDesign.getColumnModel().getColumn(NOME).setPreferredWidth(30);
				tabelaFornecedorDesign.getColumnModel().getColumn(CNPJ).setPreferredWidth(40);
				tabelaFornecedorDesign.getColumnModel().getColumn(CPF).setPreferredWidth(40);
				tabelaFornecedorDesign.getColumnModel().getColumn(EMAIL).setPreferredWidth(40);
				tabelaFornecedorDesign.getColumnModel().getColumn(TELEFONE).setPreferredWidth(20);
				
				scrollPane.setViewportView(tabelaFornecedorDesign);

	}


	private List<Fornecedor> carregarListaFornecedor(int paginaAtual, int registrosPorPagina) {
		FornecedorService fornecedorService = new FornecedorService();
		return fornecedorService.carregarListaFornecedor(( ( paginaAtual - 1) * registrosPorPagina),registrosPorPagina);	//busca paginada	
	}
	
	@SuppressWarnings("unused")
	private int buscarTotalRegistrosFornecedores() {
		return fornecedorService.countTotalRegistrosFornecedor();
	}
	...
	private void incluirNovoRegistro() {
		FornecedorFrame fornecedorFrame = new FornecedorFrame(new JFrame(),true,ProcessamentoDados.INCLUIR_REGISTRO, tabelaFornecedorDesign, tabelaFornecedorModel,0);
	...
	private void alterarRegistroCadastrado() {
		Integer linha = 0;
		if(tabelaFornecedorDesign.getSelectedRow() != -1 && tabelaFornecedorDesign.getSelectedRow() < tabelaFornecedorModel.getRowCount()) {
			linha = tabelaFornecedorDesign.getSelectedRow();
			FornecedorFrame fornecedorFrame = new FornecedorFrame(new JFrame(),true,ProcessamentoDados.INCLUIR_REGISTRO, tabelaFornecedorDesign, tabelaFornecedorModel,linha);
			fornecedorFrame.setLocationRelativeTo(null);
			fornecedorFrame.setVisible(true);
		}
		else {
	...
	private void excluirRegistroCadastrado() {
		...
		if(tabelaFornecedorDesign.getSelectedRow() != -1 && tabelaFornecedorDesign.getSelectedRow() < tabelaFornecedorModel.getRowCount()) {
			linha = tabelaFornecedorDesign.getSelectedRow();
			FornecedorFrame fornecedorFrame = new FornecedorFrame(new JFrame(),true,ProcessamentoDados.EXCLUIR_REGISTRO, tabelaFornecedorDesign, tabelaFornecedorModel,linha);
			fornecedorFrame.setLocationRelativeTo(null);
			fornecedorFrame.setVisible(true);
		}
		else {
	...
	private void consultarRegistroCadastrado() {
		...
		if(tabelaFornecedorDesign.getSelectedRow() != -1 && tabelaFornecedorDesign.getSelectedRow() < tabelaFornecedorModel.getRowCount()) {
			linha = tabelaFornecedorDesign.getSelectedRow();
			FornecedorFrame fornecedorFrame = new FornecedorFrame(new JFrame(),true,ProcessamentoDados.CONSULTAR_REGISTRO, tabelaFornecedorDesign, tabelaFornecedorModel,linha);
			fornecedorFrame.setLocationRelativeTo(null);
			fornecedorFrame.setVisible(true);
	...
	protected void filtraNomeFornecedor() {
		...
		String buscarNomeFornecedor = textFieldBuscarNomeFornecedor.getText();
		try {
	...
	private void initComponents() {
		...
		textFieldBuscarNomeFornecedor = new JTextField();
		textFieldBuscarNomeFornecedor.getDocument().addDocumentListener(new DocumentListener() {
			@Override
		...
		textFieldBuscarNomeFornecedor.setColumns(10);
		textFieldBuscarNomeFornecedor.setBounds(518, 5, 225, 20);
		getContentPane().add(textFieldBuscarNomeFornecedor);
		...
		btnPrimeiro.setIcon(new ImageIcon(TabelaFornecedor.class.getResource("/imagens/go-first.png")));
		...
		btnAnterior.setIcon(new ImageIcon(TabelaFornecedor.class.getResource("/imagens/go-previous.png")));
		...
		btnProximo.setIcon(new ImageIcon(TabelaFornecedor.class.getResource("/imagens/go-next.png")));
		...
		btnUltimo.setIcon(new ImageIcon(TabelaFornecedor.class.getResource("/imagens/go-last.png")));
		...
		btnInclusao.setIcon(new ImageIcon(TabelaFornecedor.class.getResource("/imagens/book_add.png")));
		...
		btnAlteracao.setIcon(new ImageIcon(TabelaFornecedor.class.getResource("/imagens/book_edit.png")));
		...
		btnExclusao.setIcon(new ImageIcon(TabelaFornecedor.class.getResource("/imagens/book_delete.png")));
		...
		btnConsulta.setIcon(new ImageIcon(TabelaFornecedor.class.getResource("/imagens/search.png")));
		...
		btnFechar.setIcon(new ImageIcon(TabelaFornecedor.class.getResource("/imagens/iconFechar.png")));
		...
		tabelaFornecedorDesign = new JTable();
		scrollPane.setViewportView(tabelaFornecedorDesign);
	...
	public TabelaFornecedorModel getTabelaFornecedorModel() {
		return tabelaFornecedorModel;
	}

	public void setTabelaFornecedorModel(TabelaFornecedorModel tabelaFornecedorModel) {
